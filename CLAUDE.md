...
classe "l'app non parte fuori dal profilo test"). Esegue tutte le aree, non si ferma al primo errore e ritorna exit-code ≠ 0 se una qualsiasi suite è
rossa (`./run-tests.sh` per tutto, oppure `./run-tests.sh backend|frontend|infra|compliance|smoke`).
...
**epiche evolutive (evo)** `12-ready-for-ai-mcp … 22-refactor-membership-model` (lavoro post-base: epiche 12–19 formalizzate dal backlog, change `0064`; epica 20 = suite end-to-end di piattaforma + processo di copertura; epica 21 = catalogo app & UX backoffice, change `0066`; epica 22 = rifacimento del modello di appartenenza, change `0087` — **unica area con sottocartelle** `epic/ story/ task/ prototype/`, e **supera l'epica 14**; comprende anche il superamento del vincolo «una persona → un solo account», storie 0116–0118). **In questa epica il collaudo visivo delle singole storie è sospeso** per decisione dello sviluppatore: si esegue una volta a chiusura, con la guida unica di `0120` — motivo, cosa resta valido e quando decade nel §9 di [E22-00](docs/usecases/22-refactor-membership-model/epic/E22-00-rifacimento-modello-appartenenza.md).
...
- Catalogo: **60 use case base** (0001–0060) + **60 storie evo** (0061–0120, epiche 12–22: 29 dalla change `0064`, 5 dell'epica 20 test e2e, 3 dell'epica 21 catalogo/UX, **23 dell'epica 22** rifacimento del modello di appartenenza, change `0087` — le ultime due, `0119` responsività del backoffice e `0120` guida di collaudo manuale unica, aggiunte dopo il collaudo del lotto `0095`–`0099`) = **120** in tutto.
  ...
  **0060** (localizzazione UI app a 5 lingue) scorporato dalla change `0054-use-case-0053-…` (sblocca gli screenshot landing per-lingua).
