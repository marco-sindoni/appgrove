...
> due collaudi Playwright di `frontend/apps/backoffice/e2e/seats.spec.ts` e dai collaudi di componente di
> `MembersPage.test.tsx` — qui si guardano con gli occhi, che è un'altra cosa.
...
| 0.2 | `./dev.sh migrate` | Le migrazioni girano fino a `V22` senza errori (`app-start.sh` le applica già; questo passo serve solo se si parte da una banca dati vecchia). |
...
**Esito reale** (banca dati appena seminata più le tracce della suite di piattaforma):

```json
{"usedSeats":9,"composition":{"active":6,"suspended":0,"pendingInvitations":3},"currency":"EUR",
 "freeSeats":3,"paidSeats":6,"dueCents":1794,"paidQuantity":0,
 "currentBand":{"fromSeat":4,"toSeat":10,"unitPriceCents":299},
 "next":{"seatNumber":10,"unitPriceCents":299,"dueCentsAfter":2093,"chargeCents":2093,
         "cheaperThanPrevious":false},
 "pendingReduction":false,"hasSubscription":false}
```

`6 + 0 + 3 = 9`: la somma torna. `dueCents = 1794 = 6 × 299`: i primi tre posti sono compresi, gli altri
sei costano 2,99 € l'uno. `403` e `401` come atteso; `200` sul listino con il token del collaboratore.

> **`paidQuantity: 0` con sei posti a pagamento non è un difetto**, ed è il punto aperto tracciato in UC
> 0113: quelle sei persone sono entrate quando il posto era gratuito, quindi nessuno le ha mai pagate. Il
> `chargeCents: 2093` è la conseguenza corretta — è la differenza fra il dovuto dei dieci posti e il
> dovuto di zero posti pagati. Su un account nuovo (§5 di un conto appena creato) il primo addebito sarebbe
> di 2,99 €.
...
Serve un account **nuovo**, perché quello di prova è già oltre la franchigia. Il modo più corto è
registrarne uno dall'interfaccia (`https://app.local.appgrove.app/signup`, l'email si legge su Mailpit
`http://localhost:8025`); in alternativa si legge la regola sull'account esistente, come sotto.

| # | Azione | Risultato atteso |
|---|---|---|
| 3.1 | Sull'account nuovo: `curl -sk .../me/seats -H "Authorization: Bearer $TOKEN_NUOVO"` | `usedSeats: 1` (solo l'owner), `dueCents: 0`, `paidSeats: 0`, `hasSubscription: false`, e `next.unitPriceCents: 0` — **il secondo e il terzo posto sono compresi**. |
| 3.2 | Invitare due persone da quell'account, poi `docker compose -f dev/docker-compose.yml exec -T postgres psql -U appgrove -d appgrove -c "select count(*) from platform.subscription where app_id = '$SEATS_APP';"` | `0`. **Dentro la franchigia l'abbonamento dei posti non esiste affatto** — non esiste «con quantità zero»: la differenza conta, perché un abbonamento a zero sarebbe una riga di fatturazione senza fatturazione. |
| 3.3 | Sull'account di prova: `docker compose -f dev/docker-compose.yml exec -T postgres psql -U appgrove -d appgrove -c "select from_seat, to_seat, unit_price_cents from platform.seat_pricing_band b join platform.seat_pricing_version v on v.id = b.version_id order by from_seat;"` | Cinque fasce, la prima `1 → 3` a **tariffa 0**. La franchigia è una **riga di listino**, non una condizione nel programma. |

**Esito reale**: il passo 3.3 è stato eseguito e conferma le cinque fasce `1–3 = 0`, `4–10 = 299`,
`11–50 = 199`, `51–100 = 99`, `101–∞ = 49`. I passi 3.1/3.2 richiedono la registrazione di un conto nuovo
dall'interfaccia (passo **visivo**) e non sono stati eseguiti: la stessa regola è provata dal collaudo
`SeatPurchaseApiTest.dentroLaFranchigiaNonNasceAlcunAbbonamento`, che parte da un account pulito.
...
| # | Azione | Risultato atteso |
|---|---|---|
| 4.1 | Leggere lo stato di partenza: `curl -sk .../me/seats -H "Authorization: Bearer $TOKEN_OWNER"` | Annotare `usedSeats`, `paidQuantity` e `next.chargeCents`. **Si verifica la regola, non un numero assoluto**: il conto di prova cambia con le altre suite. |
| 4.2 | `curl -sk -w "\nHTTP %{http_code}\n" -X POST https://api.local.appgrove.app/api/platform/v1/invitations -H "Authorization: Bearer $TOKEN_OWNER" -H 'content-type: application/json' -d '{"email":"collaudo-0098@acme.test"}'` | `201` con `id`, `email`, `status: "pending"`, `expiresAt` e il **token grezzo**. Nessun campo che riguardi l'addebito: chi invita non ha bisogno del riferimento della transazione. |
| 4.3 | `docker compose -f dev/docker-compose.yml exec -T postgres psql -U appgrove -d appgrove -c "select tenant_id, quantity, status, current_period_start is not null as ha_periodo, paddle_subscription_id is not null as ha_riferimento from platform.subscription where app_id = '$SEATS_APP';"` | **Una** riga per l'account: `quantity` = posti a pagamento (uno in più di prima), `status = active`, periodo valorizzato, riferimento del fornitore valorizzato. |
| 4.4 | `docker compose -f dev/docker-compose.yml exec -T postgres psql -U appgrove -d appgrove -c "select email, status, seat_charge_ref is not null as ha_addebito from platform.invitations where email = 'collaudo-0098@acme.test';"` | Una riga, `status = pending`, `ha_addebito = t`: **il posto di quell'invito è pagato**, e la riga dice con che cosa. |
| 4.5 | `curl -sk .../me/seats -H "Authorization: Bearer $TOKEN_OWNER"` | `usedSeats` salito di uno (**il posto è occupato dall'invito, non dall'accettazione**), `paidQuantity` pari a `paidSeats`, `hasSubscription: true`. |
| 4.6 | `grep -a "seats.charge" dev/.run/core.log \| tail -2` | Una riga `seats.charge.accepted` con `target`, `quantity`, `delta_cents`, `due_cents` e il riferimento della transazione. |

**Esito reale**: `201`; l'abbonamento è nato con `quantity = 7`, periodo e riferimento valorizzati;
l'invito porta il suo addebito; il riquadro è passato da 9 a 10 posti con `dueCents: 2093` e
`paidQuantity: 7`. E qui si è visto **il caso del confine di fascia**: a dieci posti occupati il prossimo
costa `199` invece di `299`, con `cheaperThanPrevious: true` e `dueCentsAfter: 2292` — il costo del posto in
più scende, il totale sale comunque.
...
| # | Azione | Risultato atteso |
|---|---|---|
| 5.1 | `INV=$(docker compose -f dev/docker-compose.yml exec -T postgres psql -U appgrove -d appgrove -tAc "select id from platform.invitations where email='collaudo-0098@acme.test'")` · `curl -sk -o /dev/null -w "%{http_code}\n" -X DELETE "https://api.local.appgrove.app/api/platform/v1/invitations/$INV" -H "Authorization: Bearer $TOKEN_OWNER"` | `204`. |
...
| 5.4 | `docker compose -f dev/docker-compose.yml exec -T postgres psql -U appgrove -d appgrove -c "select email, seat_charge_ref is null as senza_addebito from platform.invitations where email like 'collaudo-0098%' order by email;"` | Il primo invito ha l'addebito, il **secondo no** (`senza_addebito = t`): non si paga due volte lo stesso posto nello stesso periodo. E `quantity` sull'abbonamento è **rimasta uguale**. |

**Esito reale**: `204`, poi il riquadro con `usedSeats: 9`, `paidQuantity: 7` e `next.chargeCents: 0`; il
reinvito `201`; `quantity` ancora `7`; il secondo invito senza riferimento all'addebito. Esattamente la
lettura della permanenza minima mensile: nessun rimborso, posto riutilizzabile.
...
`fatture`, `legacy`, `notes`, `teams`. Il 6.5 resta visivo.

---

## 7. Senza addebito riuscito l'invito non nasce

Serve il simulatore configurato per **rifiutare**. Si riavviano le sole applicazioni con la proprietà
valorizzata (i dati restano dove sono):

```bash
./app-stop.sh --apps-only
APPGROVE_SEATS_STUB_DECLINE_REASON="carta scaduta" ./app-start.sh --no-build --no-spa
```
...
| 7.3 | `docker compose -f dev/docker-compose.yml exec -T postgres psql -U appgrove -d appgrove -c "select quantity from platform.subscription where app_id = '$SEATS_APP';"` | Il valore di prima, **invariato**. |
...
| 7.5 | Rimettere il simulatore ad accettare: `./app-stop.sh --apps-only && ./app-start.sh --no-build --no-spa` | Gli invii tornano a riuscire. |

**Esito reale**: tutto come atteso.
...
docker compose -f dev/docker-compose.yml exec -T postgres psql -U appgrove -d appgrove -c \
  "delete from platform.invitations where email like 'collaudo-0098%' or email like 'rifiutato-0098%';
   delete from platform.subscription where app_id = '22c25c07-0247-3196-8d05-a2d26587295a';"
```

**Esito reale**: eseguita; il riquadro è tornato a `usedSeats: 9`, `paidQuantity: 0`,
`hasSubscription: false`, cioè allo stato di partenza.
...
| 9.5 | Fermare il servizio `core` (`./app-stop.sh --apps-only`) e ricaricare la pagina | Il riquadro dice «Non riusciamo a leggere il costo dei posti. Finché non lo sappiamo non puoi invitare nessuno…» con un pulsante **Riprova**, e il pulsante **Invia invito** è **spento**. Mai invitare alla cieca. |
