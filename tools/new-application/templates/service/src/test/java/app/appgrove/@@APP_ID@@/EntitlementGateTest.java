 ...
 * (creazione record) è bloccato con 402 quando il tenant non ha accesso. L'endpoint di stato quota,
...
class EntitlementGateTest {
    ...
    void gatedEndpointReturns402WhenNotEntitled() {
        ...
        given().header("Authorization", "Bearer " + TestTokens.withTenant(TENANT, "owner"))
                ...
                .body(Map.of("contactName", "Senza accesso"))
                .when().post(ITEMS)
                .then().statusCode(402)
    ...
    void gatedEndpointAllowsWhenEntitled() {
        ...
        given().header("Authorization", "Bearer " + TestTokens.withTenant(TENANT, "owner"))
                .contentType(ContentType.JSON)
                .body(Map.of("contactName", "Con accesso"))
                .when().post(ITEMS)
                .then().statusCode(201);
    }
