...
| `cognito.credentials` | Authentication credentials (password hash) and MFA/TOTP secrets | Amazon Cognito user pool (eu-west-1); locally a dev-only stub (`auth-local`, out of RoPA scope) | Registered users | User authentication and access protection (MFA) | Contract (art. 6(1)(b) GDPR) | While the account is active; removed when the Cognito user is deleted |
