...
public final class TestTokens {
    ...
    public static String withTenant(String tenantId, String... roles) {
        return withTenantAndUser(tenantId, "sub-" + tenantId, roles);
    }

    /** {@code sub} (= user id) del token prodotto da {@link #withTenant} per quel tenant. */
    public static String subOf(String tenantId) {
        return "sub-" + tenantId;
    }

    /**
     * Token con tenant e utente ({@code sub}) espliciti: serve ai test multi-utente dell'app crm, dove
     * più membri dello stesso account (stesso {@code tenant_id}, {@code sub} diversi) hanno o non hanno
     * un posto assegnato.
     */
    public static String withTenantAndUser(String tenantId, String userId, String... roles) {
        return access()
                .upn("user-" + userId)
                .subject(userId)
