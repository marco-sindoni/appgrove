...
/**
 * Il contratto dei tre ruoli, reso <b>vero</b> per il Mini-CRM (UC 0101 §8): il documento delle operazioni
 * e il codice devono dire la stessa cosa, nelle due direzioni.
 *
...
class AppOperationsContractTest {
    ...
    private final CrmOperationsContract contract = new CrmOperationsContract();

    @Test
    void ilDocumentoDelleOperazioniCoincideConIlCodice() {
        AppOperationsContractVerifier.verify(contract, "app.appgrove.crm");
    }
    ...
    void ilDocumentoParlaDiQuestaApplicazione() {
        assertEquals("crm", contract.appId());
    }
}
